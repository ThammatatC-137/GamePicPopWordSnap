...
public class QuizManager : MonoBehaviour
{
    ...
    [SerializeField]
    public QuizDataScriptable questionData;
    [SerializeField]
    ...
    [SerializeField]
    private GameObject gameLoseObjectV2;
    ...
    private void Start()
    {
        Timer.remainingTime = 121;
        // ตรวจสอบว่ายังมีคำถามในรายการหรือไม่
        if (currentQuestionIndex < questionData.questions.Count)
        {
            questionData.questions = ShuffleQuestions(questionData.questions); // สุ่มลำดับของคำถาม
    ...
    public void SelectedOption(WordData wordData)
    {
        ...
        if (currentAnswerIndex >= answerWord.Length)
        {
            ...
            else
            {
                ...
                playIncorrectSound();
                ResetQuestion(); // Reset the question and options
                foreach (int index in selectedWordIndex)
                {
                    optionsWordArray[index].gameObject.SetActive(true);
                    currentAnswerIndex--;
                }
                selectedWordIndex.Clear(); // Clear the list of selected word indices
                Timer.remainingTime = Timer.remainingTime - 5;
            }
        }
    }
